  ...
  return (
    <div className="section-padding bg-primary-50">
      <div className="container-custom max-w-4xl">
        ...
        <div className={`${isRejected ? 'bg-red-50 border-red-300 ring-red-300' : isGreenStatus ? 'bg-green-50 border-green-300 ring-green-300' : 'bg-yellow-50 border-yellow-300 ring-yellow-300'} border-2 ring-2 p-4 md:p-5 rounded-lg text-center mb-4`}>
          <div className={`${isRejected ? 'bg-red-100' : isGreenStatus ? 'bg-green-100' : 'bg-yellow-100'} w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3`}>
            <svg className={`w-6 h-6 ${isRejected ? 'text-red-600' : isGreenStatus ? 'text-green-600' : 'text-yellow-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <h2 className={`text-xl md:text-2xl font-serif mb-1 ${isRejected ? 'text-red-900' : isGreenStatus ? 'text-green-900' : 'text-yellow-900'}`}>
            {isRejected ? 'Booking Ditolak' : isGreenStatus ? 'Booking Dikonfirmasi' : 'Pembayaran Sedang Diverifikasi'}
          </h2>
          <p className={`text-sm md:text-base mb-2 max-w-xl mx-auto [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical] overflow-hidden ${isRejected ? 'text-red-700' : isGreenStatus ? 'text-green-700' : 'text-yellow-700'}`}>
            {isRejected
          ...
          {isRejected ? (
            ...
            <p className={`text-xs md:text-sm ${isGreenStatus ? 'text-green-600' : 'text-yellow-600'}`}>
              {isPendingStatus
        ...
        <div className="bg-green-50 border-2 border-green-400 p-4 md:p-5 rounded-lg mb-4 shadow-lg">
          <div className="text-center mb-3">
            ...
            <h3 className="text-lg md:text-xl font-bold text-green-900 mb-1 [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical] overflow-hidden">Kirim Konfirmasi via WhatsApp</h3>
            <p className="text-green-700 mb-3 text-xs md:text-sm">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-6 py-3 md:py-4 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-all transform hover:scale-105 active:scale-95 font-bold text-lg shadow-xl md:text-xl"
          >
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm mb-4">
          <h3 className="text-base font-semibold text-primary-900 mb-3">Detail Pesanan Anda:</h3>
