...
import { differenceInDays, format, eachDayOfInterval, isSameDay, addDays, subDays } from 'date-fns';
...
const BookingCalendarPage: React.FC = () => {
  ...
  // Each selected date represents 1 night: check-in 12:00, check-out 12:00 next day
  const sortedDates = [...selectedDates].sort((a, b) => a.getTime() - b.getTime());
  const derivedCheckIn = sortedDates.length >= 1 ? sortedDates[0] : null;
  const derivedCheckOut = derivedCheckIn ? addDays(derivedCheckIn, sortedDates.length) : null;
  const numberOfNights = sortedDates.length;
  ...
  // Handle date selection - contiguous + confirmation modal
  const handleDateToggle = (dates: Date[]) => {
    // Calendar calls this with updated dates list
    // Process the change to implement our logic

    if (dates.length === 0) {
      // All dates deselected
      setSelectedDates([]);
      return;
    }

    const wasRemoved = dates.length < selectedDates.length;
    const addedDate = dates.find(d => !selectedDates.some(sd => isSameDay(d, sd)));

    if (wasRemoved && !addedDate) {
      // Date was removed - Calendar already handles this, just sync
      setSelectedDates(dates);
      return;
    }

    // Date was added - handle contiguous logic
    if (!addedDate) {
      // No new date, just sync
      setSelectedDates(dates);
      return;
    }

    // Check if added date is contiguous with last selected date
    const sorted = [...selectedDates].sort((a, b) => a.getTime() - b.getTime());
    if (sorted.length === 0) {
      // First date selected - Calendar already handles this
      setSelectedDates(dates);
      return;
    }

    const lastSelectedDate = sorted[sorted.length - 1];
    const diffInDays = differenceInDays(addedDate, lastSelectedDate);

    if (Math.abs(diffInDays) === 1) {
      // Contiguous - Calendar already handles this
      setSelectedDates(dates);
      return;
    }

    // Non-contiguous - show confirmation modal
    setPendingDateToAdd(addedDate);
  ...
  return (
    <div className="section-padding bg-primary-50">
          ...
          <div className="relative bg-white max-w-md w-full mx-4 p-8 shadow-xl">
            ...
            <div className="space-y-4">
              <button onClick={handleBookViaWebsite} className="w-full flex items-center gap-4 p-4 border-2 border-primary-200 hover:border-gold-600 rounded transition-colors text-left group">
                <div className="w-12 h-12 bg-primary-100 rounded-full flex items-center justify-center flex-shr-0 group-hover:bg-gold-50">
                  <svg className="w-6 h-6 text-primary-900" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03 3-9s1.343-9 3-9m0 18c-1.657 0 3-4.03-3-9m9 9c1.657 0 3-4.03 3-9s-1.343-9 3-9m0 18c-1.657 0 3-4.03 3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg>
                </div>
              ...
              <button onClick={handleBookViaWhatsApp} className="w-full flex items-center gap-4 p-4 border-2 border-primary-200 hover:border-green-500 rounded transition-colors text-left group">
                <div className="w-12 h-12 bg-green-50 rounded-full flex items-center justify-center flex-shr-0 group-hover:bg-green-100">
                  <svg className="w-6 h-6 text-green-600" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335 11.893-11.893a11.821 11.821 0 00-9-9-9m9 9c1.657 0 3-4.03 3-9s1.343-9 3-9m-9 9c1.657 0 3-4.03 3-9s1.343-9 3-9m0 18c-1.657 0 3-4.03 3-9s1.343-9 3-9m9 9a9 9 0 019-9" /></svg>
                </div>
      ...
      {/* Non-Contiguous Date Confirmation Modal */}
      {showConfirmModal && pendingDateToAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          ></div>
          <div className="relative bg-white max-w-md w-full mx-4 p-8 shadow-xl rounded-lg">
            <button
              onClick={handleCancelReplace}
            ...
            <div className="text-center">
              <div className="w-16 h-16 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 6v.01m-6 6H12m0-6v6m0-6v6m0-6v6m0-6v.01" />
                </svg>
              </div>
