/**
 * Direct PromoService API Client
...
export async function applyPromoBatch(data: ApplyBatchRequest): Promise<ApplyBatchResponse> {
  return apiRequest<ApplyBatchResponse>('/promo/apply-batch', {
    method: 'POST',
    body: JSON.stringify(data),
  });
}
...
/**
 * Create a new promo code (REQUIRES AUTHENTICATION)
 */
...
/**
 * List all promo codes with optional filters (REQUIRES AUTHENTICATION)
 */
...
/**
 * Get a single promo by code (REQUIRES AUTHENTICATION)
 */
...
/**
 * Update an existing promo (REQUIRES AUTHENTICATION)
 */
...
/**
 * Soft-delete (deactivate) a promo (REQUIRES AUTHENTICATION)
 */
...
/**
 * View usage history for a promo code (REQUIRES AUTHENTICATION)
 */
...
/**
 * Get all promo codes belonging to logged-in affiliate (REQUIRES AUTHENTICATION)
 */
